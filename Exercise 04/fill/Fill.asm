...
(MAINLOOP)
    // first, get the address of screen
    @SCREEN
    D = A
    
    // then, intialize index to screen's address
    @ind
    M = D
    ...
    (BLACKLOOP)
        // paint the pixel's index pointing at (black)
        @ind
        A=M
        M=-1
        ...
        // check if done coloring (if ind == @KBD)
        @ind
        D = M
        @KBD
        D = D - A
    ...
    (WHITELOOP)
        // paint the pixel's index pointing at (white)
        @ind
        A=M
        M=0
